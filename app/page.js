...
const LC=({l,v,ch,tp,cl})=><div style={{background:C.white,borderRadius:10,padding:"18px 20px",border:`1px solid ${C.bd}`,flex:1,minWidth:140,boxShadow:C.sh,textAlign:"center"}}><div style={{fontSize:11,color:C.tl,fontWeight:700,marginBottom:6,textTransform:"uppercase",letterSpacing:"0.06em",fontFamily:F}}>{l}</div><div style={{fontSize:32,fontWeight:700,color:C.t,fontFamily:F,lineHeight:1.1,marginBottom:6}}>{v.toLocaleString()}</div><div style={{display:"flex",flexDirection:"column",gap:2,alignItems:"center"}}>{ch?.[0]!==undefined&&cl?.[0]&&<div style={{display:"flex",alignItems:"center",gap:4}}><span style={{fontSize:9,color:C.tl,fontWeight:700,fontFamily:F}}>{cl[0]}</span><A v={ch[0]} z={12}/></div>}{ch?.[1]!==undefined&&cl?.[1]&&<div style={{display:"flex",alignItems:"center",gap:4}}><span style={{fontSize:9,color:C.tl,fontWeight:700,fontFamily:F}}>{cl[1]}</span><A v={ch[1]} z={12}/></div>}</div></div>;

const CC=({l,v,ch,ci,cl})=>{const c=ch?.[ci];return<div style={{background:C.white,borderRadius:10,padding:"18px 20px",border:`1px solid ${C.bd}`,flex:1,minWidth:140,boxShadow:C.sh,textAlign:"center"}}><div style={{fontSize:11,color:C.tl,fontWeight:700,marginBottom:6,textTransform:"uppercase",letterSpacing:"0.06em",fontFamily:F}}>{l}</div><div style={{fontSize:32,fontWeight:700,color:C.t,fontFamily:F,lineHeight:1.1,marginBottom:6}}>{v.toLocaleString()}</div>{c!==undefined&&<div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:4}}><span style={{fontSize:9,color:C.tl,fontWeight:700,fontFamily:F}}>{cl}</span><A v={c} z={12}/></div>}</div>};
...
const BC=({ic,la,hl,ch})=><div style={{background:`linear-gradient(135deg,${C.bc},${C.bl})`,borderRadius:12,padding:"18px 22px",border:`1px solid ${C.bb}`,overflow:"hidden",boxShadow:"0 2px 8px rgba(0,0,0,0.15)"}}><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}><span style={{fontSize:18}}>{ic}</span><span style={{fontSize:15,fontWeight:700,color:"#fff",fontFamily:F}}>{la}</span></div>{ch}{hl&&<div style={{marginTop:10,paddingTop:10,borderTop:"1px solid rgba(255,255,255,0.1)",fontSize:12}}><span style={{color:"rgba(255,255,255,0.4)",fontSize:10,textTransform:"uppercase",fontWeight:700,fontFamily:F}}>Highlight: </span><span style={{color:"#fff",fontWeight:700,fontFamily:F}}>{hl}</span></div>}</div>;
      ...
      <div style={{background:C.white,padding:"0 24px",height:64,display:"flex",alignItems:"center",justifyContent:"space-between",borderBottom:`1px solid ${C.bd}`,boxShadow:"0 1px 4px rgba(0,0,0,0.04)"}}>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          ...
          <div style={{position:"relative"}}>
            <button onClick={()=>{sCm(!cm);sDm(false)}} style={{background:"#f0f2f5",border:`1px solid ${C.bd}`,borderRadius:8,padding:"8px 16px",color:C.t,fontSize:14,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",gap:8,fontFamily:F}}>{cl.n} <span style={{fontSize:10,color:C.tl}}>▼</span></button>
            {cm&&<div style={{position:"absolute",top:"calc(100% + 5px)",left:0,background:C.white,border:`1px solid ${C.bd}`,borderRadius:10,padding:"6px 0",zIndex:200,width:260,maxHeight:400,overflowY:"auto",boxShadow:"0 8px 30px rgba(0,0,0,0.12)"}}>{["Goode Motor","Juneau Auto Mall","Independent"].map(g=><div key={g}><div style={{padding:"8px 14px",fontSize:10,fontWeight:700,color:C.tl,textTransform:"uppercase",letterSpacing:"0.08em",fontFamily:F}}>{g}</div>{CL.filter(c=>c.g===g).map(c=><button key={c.id} onClick={()=>{sCl(c);sCm(false)}} style={{display:"block",width:"100%",textAlign:"left",padding:"8px 14px",border:"none",cursor:"pointer",background:c.id===cl.id?C.cyanL:"transparent",color:c.id===cl.id?C.cyanD:C.t,fontSize:13,fontWeight:700,fontFamily:F}}>{c.n}</button>)}</div>)}</div>}
          </div>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <div style={{position:"relative"}}>
            <button onClick={()=>{sDm(!dm);sCm(false)}} style={{background:"#f0f2f5",border:`1px solid ${C.bd}`,borderRadius:8,padding:"8px 16px",color:C.t,fontSize:13,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",gap:8,fontFamily:F}}>📅 {dl} <span style={{fontSize:10,color:C.tl}}>▼</span></button>
            {dm&&<div style={{position:"absolute",top:"calc(100% + 5px)",right:0,background:C.white,border:`1px solid ${C.bd}`,borderRadius:10,padding:"6px 0",zIndex:200,width:220,boxShadow:"0 8px 30px rgba(0,0,0,0.12)"}}>{PR.map(p=><button key={p.v} onClick={()=>{sDt(p.v);sDm(false)}} style={{display:"block",width:"100%",textAlign:"left",padding:"8px 14px",border:"none",cursor:"pointer",background:p.v===dt?C.cyanL:"transparent",color:p.v===dt?C.cyanD:C.t,fontSize:13,fontWeight:700,fontFamily:F}}>{p.l}<div style={{fontSize:10,color:C.tl,marginTop:1,fontFamily:F}}>{p.c.join(" • ")}</div></button>)}</div>}
          </div>
