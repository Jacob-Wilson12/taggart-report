...
const ts={background:C.white,border:`1px solid ${C.bd}`,borderRadius:8,fontSize:12,color:C.t};
const A=({v,s="%",z=13})=><span style={{display:"inline-flex",alignItems:"center",gap:3,color:v>0?C.g:v<0?C.r:C.tl,fontSize:z,fontWeight:600}}>{v>0?"▲":v<0?"▼":"—"} {Math.abs(v)}{s}</span>;
const SH=({title,sub})=><div style={{marginBottom:10}}><h2 style={{fontSize:16,fontWeight:700,color:C.t,margin:0,fontFamily:"'Instrument Serif', Georgia, serif"}}>{title}</h2>{sub&&<p style={{fontSize:11,color:C.tl,margin:"2px 0 0"}}>{sub}</p>}</div>;
...
const LC=({l,v,ch,tp,cl})=><div style={{background:C.white,borderRadius:10,padding:"18px 20px",border:`1px solid ${C.bd}`,flex:1,minWidth:140,boxShadow:C.sh,textAlign:"center"}}><div style={{fontSize:11,color:C.tl,fontWeight:600,marginBottom:6,textTransform:"uppercase",letterSpacing:"0.06em"}}>{l}</div><div style={{fontSize:32,fontWeight:700,color:C.t,fontFamily:"'Instrument Serif', Georgia, serif",lineHeight:1.1,marginBottom:6}}>{v.toLocaleString()}</div><div style={{display:"flex",flexDirection:"column",gap:2,alignItems:"center"}}>{ch?.[0]!==undefined&&cl?.[0]&&<div style={{display:"flex",alignItems:"center",gap:4}}><span style={{fontSize:9,color:C.tl,fontWeight:500}}>{cl[0]}</span><A v={ch[0]} z={12}/></div>}{ch?.[1]!==undefined&&cl?.[1]&&<div style={{display:"flex",alignItems:"center",gap:4}}><span style={{fontSize:9,color:C.tl,fontWeight:500}}>{cl[1]}</span><A v={ch[1]} z={12}/></div>}</div></div>;

const CC=({l,v,ch,ci,cl})=>{const c=ch?.[ci];return<div style={{background:C.white,borderRadius:10,padding:"18px 20px",border:`1px solid ${C.bd}`,flex:1,minWidth:140,boxShadow:C.sh,textAlign:"center"}}><div style={{fontSize:11,color:C.tl,fontWeight:600,marginBottom:6,textTransform:"uppercase",letterSpacing:"0.06em"}}>{l}</div><div style={{fontSize:32,fontWeight:700,color:C.t,fontFamily:"'Instrument Serif', Georgia, serif",lineHeight:1.1,marginBottom:6}}>{v.toLocaleString()}</div>{c!==undefined&&<div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:4}}><span style={{fontSize:9,color:C.tl,fontWeight:500}}>{cl}</span><A v={c} z={12}/></div>}</div>};

const WC=({l,v,ch,pre="",suf="",tip})=><div style={{background:C.white,borderRadius:10,padding:"16px 18px",border:`1px solid ${C.bd}`,flex:1,minWidth:130,boxShadow:C.sh,textAlign:"center"}}><div style={{fontSize:11,color:C.tl,fontWeight:600,marginBottom:5,textTransform:"uppercase",letterSpacing:"0.06em"}}>{l}</div><div style={{fontSize:28,fontWeight:700,color:C.t,fontFamily:"'Instrument Serif', Georgia, serif",lineHeight:1.1,marginBottom:4}}>{pre}{typeof v==="number"?v.toLocaleString():v}{suf}</div>{ch!==undefined&&<A v={ch} z={12}/>}{tip&&<div style={{fontSize:9,color:C.tl,marginTop:3}}>{tip}</div>}</div>;

const BM=({l,v,ch,pre="",suf="",ci})=>{const c=ch?ch[ci]:undefined;return<div style={{flex:1,minWidth:100,textAlign:"center",padding:"12px 6px"}}><div style={{fontSize:10,color:"rgba(255,255,255,0.45)",fontWeight:600,textTransform:"uppercase",letterSpacing:"0.07em",marginBottom:4}}>{l}</div><div style={{fontSize:24,fontWeight:700,color:"#fff",fontFamily:"'Instrument Serif', Georgia, serif",lineHeight:1.1,marginBottom:4}}>{pre}{typeof v==="number"?v.toLocaleString():v}{suf}</div>{c!==undefined&&<span style={{fontSize:12,fontWeight:600,color:c>0?"#6ee7b7":c<0?"#fca5a5":"rgba(255,255,255,0.4)"}}>{c>0?"▲":c<0?"▼":"—"} {Math.abs(c)}%</span>}</div>};
...
function Dashboard({ci,cls}){
  return<>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:12,marginBottom:18}}>
      ...
      <BC ic="📍" la="Google Business Profile" ch={<><div style={{display:"flex",flexWrap:"wrap"}}><BM l="Views" v={D.gbp.vi} ch={D.gbp.viC} ci={ci}/><BM l="Searches" v={D.gbp.se} ch={D.gbp.seC} ci={ci}/><BM l="Map Views" v={D.gbp.ma} ch={D.gbp.maC} ci={ci}/><BM l="Web Clicks" v={D.gbp.wc} ch={D.gbp.wcC} ci={ci}/><BM l="Calls" v={D.gbp.ca} ch={D.gbp.caC} ci={ci}/><BM l="Directions" v={D.gbp.di} ch={D.gbp.diC} ci={ci}/></div><div style={{marginTop:10,paddingTop:10,borderTop:"1px solid rgba(255,255,255,0.1)",display:"flex",gap:20,justifyContent:"center",flexWrap:"wrap"}}><span style={{fontSize:12,color:"rgba(255,255,255,0.6)"}}>⭐ <strong style={{color:"#fbbf24"}}>{D.gbp.ra}</strong> ({D.gbp.rv} reviews)</span><span style={{fontSize:12,color:"rgba(255,255,255,0.6)"}}>+{D.gbp.nr} new</span></div></>}/>
    ...
    <div style={{display:"flex",gap:14,flexWrap:"wrap"}}><Sc s={{flex:1,minWidth:260}}><SH title="Key Highlights"/><div style={{display:"flex",flexDirection:"column",gap:6}}>{[{i:"🔍",t:'"F-150 Twin Falls" Pos 1'},{i:"📢",t:"Ads CPL dropped 8.7%"},{i:"📞",t:"CallRail up 18.2%"},{i:"🎬",t:"Social engagement up 38.1%"},{i:"⚠️",t:"Rob Green Ford growing"}].map((h,i)=><div key={i} style={{display:"flex",alignItems:"flex-start",gap:8,padding:"8px 10px",borderRadius:6,background:"rgba(232,236,242,0.7)"}}><span style={{fontSize:14}}>{h.i}</span><span style={{fontSize:12,color:C.tm,lineHeight:1.4}}>{h.t}</span></div>)}</div></Sc><Sc s={{flex:1,minWidth:260}}><SH title="Coming Next Month"/><div style={{display:"flex",flexDirection:"column",gap:5}}>{[{d:"SEO",t:"Maverick page + competitor analysis"},{d:"GBP",t:"Posting 3x/week"},{d:"Ads",t:"VLA used inventory"},{d:"Meta",t:"Reels spring event"},{d:"Social",t:"Service video series"},{d:"Email",t:"Spring Service Special"}].map((x,i)=><div key={i} style={{display:"flex",alignItems:"flex-start",gap:8,padding:"8px 10px",borderRadius:6,background:"rgba(232,236,242,0.7)"}}><span style={{fontSize:10,fontWeight:700,color:C.cyanD,background:C.cyanL,padding:"2px 7px",borderRadius:3,flexShrink:0,marginTop:1}}>{x.d}</span><span style={{fontSize:12,color:C.tm,lineHeight:1.4}}>{x.t}</span></div>)}</div></Sc></div>
  </>;
}
...
export default function App(){
  ...
  return<div style={{minHeight:"100vh",background:`url('/white-brick-wall-texture-brick-with-white-filling.jpg') repeat`,backgroundColor:"#ebedf1"}}>
    {/* Header */}
    ...
    {/* Footer */}
    <div style={{padding:"18px 24px",textAlign:"center",marginTop:30,background:C.white,borderTop:`1px solid ${C.bd}`}}><div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:4}}><div style={{width:24,height:24,borderRadius:"50%",background:C.navy,display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{fontFamily:"'Permanent Marker'",fontSize:15,color:C.navy}}>TAGGART</span><span style={{fontFamily:"'Permanent Marker'",fontSize:15,color:C.cyan}}>ADVERTISING</span></div></div>
  </div>;
}
